...
## 扣减库存
扣减库存由于多线程环境的问题，很可能出现刚刚检测完可以扣除，但是真的扣除的时候已经被别的线程抢先了这种问题

1. java加锁 因为是加在代码中，所以如果是多个程序都在访问资源，就无效了，也就是集群服务器此时会出问题
2. 数据库锁 悲观锁 性能差
3. 数据库 乐观锁 没有实际的锁，而是处理并发竞态的思想 √

悲观锁：相对于乐观锁而言
1. 开启事务
2. 查询 sku.stock for update 加锁 行锁
3. update
4. 提交
...
## 延迟消息队列的不可信性
不能完全相信延迟消息队列的触发情况，但是问题在于既然我们无论如何都要规避这种情况那延迟消息队列的意义在这里似乎就变得鸡肋了
...
CreateTime+Pay + Time-Limit = expiredTime

1. 下单时就写入EndTime，固化支付时间
但是这样又会产生java中的创建时间和数据库中的创建时间时钟不统一的现象
这里是用一个新的placed_time作为下单时间的基准，createTime仍然作为行的创建时间
但是说是话，其实这个时钟差的很小很小
2. 每次查询的时候动态计算
